...
public class InnerMsgHandler implements AresClientTcpHandler {
    ...
    public void handleMsgRcv(AresTKcpContext aresTKcpContext) throws IOException {
        ...
        long roleId = msgHeader.getRoleId();
        ...
        directSendToClient(roleId, aresPacket, 0);
    }
    ...
    private void directSendToClient(long roleId, AresPacket aresPacket, int headerLen) {
        ProtoCommon.MsgHeader innerMsgHeader = aresPacket.getRecvHeader().toBuilder().setRoleId(0).build();
        //|body|
        int readableBytes = 0;
        ...
        sessionService.sendPlayerMsg(roleId, byteBufs);
